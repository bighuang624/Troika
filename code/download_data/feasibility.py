'''
python -u download_data/feasibility.py --dataset mit-states --dataset_root /mnt/nas-zhangjiakou/siteng/CZSL_data --data_root ./data
'''
import argparse
import os
...
def load_glove_embeddings(vocab):
    ...
    emb_file = os.path.join(config.data_root, 'data/glove.6B.300d.txt')
...
def clip_embeddings(model, tokenizer, words_list):
...
if __name__ == "__main__":
    ...
    save_path = os.path.join(config.data_root, f'data/feasibility_{config.dataset}.pt')
    torch.save({
